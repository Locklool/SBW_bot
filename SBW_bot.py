...
async def очистка(ctx, amount = 100):
	...
	#Команда для админов
	if 704098951002980383 in [y.id for y in ctx.author.roles] or 718059665899913238 in [y.id for y in ctx.author.roles] or 704300807331250307 in [y.id for y in ctx.author.roles]:
		await ctx.channel.purge(limit = amount)
...
async def размут(ctx, member: discord.Member):
	...
	#Команда для админов
	if 704098951002980383 in [y.id for y in ctx.author.roles] or 718059665899913238 in [y.id for y in ctx.author.roles] or 704300807331250307 in [y.id for y in ctx.author.roles]: #1-админ, 2-разраб, 3-можер
		emb = discord.Embed( title = 'Размут',colour = discord.Color.green())
		#Удаление сообщения
		await ctx.channel.purge(limit = 1)
		mute_role = discord.utils.get(member.guild.roles, id = 710578166413918220)
...
async def патреон(ctx):
	...
	emb = discord.Embed( title = 'Нажми на меня для перехода на наш патреон(платформа для спонсорства)', description = 'Это наш патреон. Здесь вы можете пожертвовать деньгу, чтобы стать спонсорм, или же просто помочь нам в развитии',colour = discord.Color.green(), url = 'https://www.youtube.com/channel/UC1S3iEfqkZeXNA3FI1mPUKw')
	#Аватар пользователя
	emb.set_footer(text = ctx.author.name, icon_url = ctx.author.avatar_url)
